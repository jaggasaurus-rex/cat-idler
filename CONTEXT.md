...
| `multiply_cat_cost_growth` | `(factor: float) -> void` | Multiplies `cat_cost_growth_rate` by `factor`, clamped to minimum 1.0 (prevents cost curve from inverting), then retroactively recalculates `next_cat_cost = Config.cat_cost_base * pow(cat_cost_growth_rate, float(cats))`; called with 0.9 on `cat_breeder_contract` completion and 0.8 on `cat_breeders_contract` completion; stacks correctly with shop `buy_breeder_contract()` regardless of order |
