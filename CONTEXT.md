...
| `cyborg_cats` | `int` | `0` | Cyborg cat population, **separate** from `cats`. Each earns `get_cyborg_multiplier()` × the full per-cat income rate and never poops. Created by `buy_cyborg_cat()` (consumes 1 normal cat). Counts toward the happiness denominator and the total population (`get_total_cats()` = `cats + cyborg_cats`), which drives the "Cats: X/MAX" HUD count and the housing cap — so converting a cat leaves the total and cap usage unchanged |
...
| `cyborg_multiplier_tier` | `int` | `0` | Index into `Config.CYBORG_MULTIPLIERS` giving the current cyborg income multiplier M; advanced by `buy_cyborg_multiplier_upgrade()` |
...
| `cyborg_cat_created` | Emitted by `buy_cyborg_cat()` after a successful conversion; Main.gd tints one existing normal CatCharacter into a cyborg and removes it from the poop loop |
...
| `get_onlypaws_cyborg_cats` | `() -> int` | Returns `cyborg_cats - get_research_cats_for(cyborg_cats)`; cyborg cats earning OnlyPaws income |
| `get_cyborg_multiplier` | `() -> float` | Returns `Config.CYBORG_MULTIPLIERS[cyborg_multiplier_tier]` (M = 2.0 / 4.0 / 8.0) |
...
| `buy_cyborg_cat` | `() -> void` | Guards `research_complete["cyborg_cats"]`, `cats >= 1`, `money >= next_cyborg_cost`; deducts money, `cats -= 1`, `cyborg_cats += 1`, multiplies `next_cyborg_cost` by `Config.CYBORG_COST_GROWTH`, calls `update_paws_rate()`, emits `cyborg_cat_created` |
| `buy_cyborg_multiplier_upgrade` | `() -> void` | Guards `cyborg_multiplier_tier + 1 < Config.CYBORG_MULTIPLIERS.size()` and `money >= Config.CYBORG_MULTIPLIER_UPGRADE_COSTS[cyborg_multiplier_tier]`; deducts cost, advances `cyborg_multiplier_tier`, calls `update_paws_rate()` |
...
| `update_paws_rate` | `() -> void` | Public (renamed from `_update_paws_rate`). Computes `rate = onlypaws_income_per_cat + onlypaws_income_per_bot * manager_bots + MEGA_BOT_INCOME_PER_CAT * mega_bots`, then `paws_income_rate = rate * (get_onlypaws_normal_cats() + get_cyborg_multiplier() * get_onlypaws_cyborg_cats())` — cyborg cats multiply the **whole** per-cat rate by M, each pool split proportionally by `research_cat_fraction`. Worked example: 20 normal + 10 cyborg, 10 bots, 5 mega-bots, M=2.0, fraction 0.0, per-mega rate 1.0 → rate=10.25, income=10.25×(20+2.0×10)=10.25×40=410.0/sec. Must be called by any caller that changes cat/cyborg count, bot/mega-bot count, cyborg tier, or `research_cat_fraction`. The no-bot fallback in `_process()` mirrors this with only the base per-cat rate |
...
| `CYBORG_MULTIPLIERS` | `Array` | `[2.0, 4.0, 8.0]` | Cyborg income multiplier M by `cyborg_multiplier_tier` index |
| `CYBORG_MULTIPLIER_UPGRADE_COSTS` | `Array` | `[5000.0, 25000.0]` | Money cost to advance the cyborg multiplier tier 0→1 and 1→2 |
...
- **Housing tier labels** (`HOUSING_LABEL_STUDIO_BASIC`, `HOUSING_LABEL_STUDIO_UPGRADED`, `HOUSING_LABEL_BEDROOM_1/2/3`) — referenced by `Config.housing_tiers` array and displayed in Main.gd's housing shop button.
...
| `_on_buy_cyborg_multiplier_button_pressed()` | Calls `GameState.buy_cyborg_multiplier_upgrade()` |
| `_on_cyborg_popup_ok_pressed()` | Hides `CyborgPopup` and unpauses the tree |
| `_on_cyborg_cat_created()` | Reacts to `GameState.cyborg_cat_created`: finds the first CatContainer child not already in `_cyborg_cat_ids`, records its id, tints it `CYBORG_TINT` (light blue), and erases it from `_cat_poop_timers` so cyborgs never poop. Node count is unchanged — a normal cat node is repurposed as a cyborg |
...
| `_on_cat_lost()` | Removes the last **non-cyborg** `CatContainer` child (scans from the back, skipping ids in `_cyborg_cat_ids`; falls back to the last child only if all remaining are cyborgs), since `cat_lost` only decrements the normal `cats` count: erases its `instance_id` from `_cat_bubble_timers`, `_cat_poop_timers`, and `_cyborg_cat_ids`, then `queue_free()`s the node. If the sweeper is in CLEANING and `_active_poops` is now empty, clears `_sweeper_target_poop` and sends it to MOVING so it doesn't chase a freed target |
...
- [x] **Cyborg Cats** — gated behind the `cyborg_cats` research (min_housing_tier 2, unlocks at 20 cats, $3,000 fund / 1,800 pts, all prior research complete). Once complete, a "Resistance Is Fur-tile" achievement popup fires once and the conversion UI (CyborgCatsLabel + MakeCyborgButton) and multiplier-upgrade shop button appear. `buy_cyborg_cat()` converts 1 normal cat into a cyborg for an escalating money cost (`CYBORG_COST_BASE` $250 × `CYBORG_COST_GROWTH` 1.4 per conversion). Cyborgs are a separate `cyborg_cats` population that multiplies the **full** per-cat income rate by M (`CYBORG_MULTIPLIERS` [2,4,8] by `cyborg_multiplier_tier`), never poop (excluded from the poop loop, tinted light blue in CatContainer), still consume cat food, and still count toward the happiness denominator (`cats + cyborg_cats`). The research/OnlyPaws slider split applies proportionally to both pools. `buy_cyborg_multiplier_upgrade()` advances the tier for `CYBORG_MULTIPLIER_UPGRADE_COSTS` [$5k, $25k]; the shop button disappears at the top tier
