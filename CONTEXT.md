...
├── CatsLabel (Label)                 ← section header; positioned first (top=20); styled via _style_as_header() in _ready() (UI_HEADER_FONT_SIZE 28 + bold); updated every _process() frame; text format "Cats: X/MAX" where X = GameState.get_total_cats() (normal + cyborg, so conversions don't change it) and MAX = get_max_cats(); modulate = RED when total > MAX, WHITE otherwise
...
├── CyborgCatsLabel (Label)              ← left column at offset_top=450; hidden until research_complete["cyborg_cats"] (one-way latch); "Cyborg Cats: X" from GameState.cyborg_cats each frame
├── MakeCyborgButton (Button)            ← left column at offset_top=482; revealed with CyborgCatsLabel; "Make Cyborg Cat ($N) [-1 cat]" cost from GameState.next_cyborg_cost; disabled when cats < 1 or unaffordable; calls buy_cyborg_cat()
├── CyborgPopup (ColorRect)              ← full-screen dark overlay; process_mode=WHEN_PAUSED; shown once when research_complete["cyborg_cats"] first true (gated by Main.gd local _cyborg_popup_shown); body = Strings.POPUP_CYBORG; pauses tree; dismiss unpauses only
...
│           └── (dynamic) _level_up_cyborg_button (Button) ← created in _ready() and added to ShopList; hidden when count_cyborgs_eligible_to_level() == 0; visible while any cyborg can advance a level; text = BTN_LEVEL_UP_CYBORG ("Level Up Cyborg → VN") where N mirrors the first eligible level from level_up_cyborg()'s scan; no money cost; calls level_up_cyborg()
...
| `cyborg_cats` | `int` | `0` | Cyborg cat population, **separate** from `cats`. Each has its own level (1/2/3) stored in `cyborg_levels`; income contribution is `Config.CYBORG_MULTIPLIERS[level-1]` × the full per-cat rate, summed per-cat. Created by `buy_cyborg_cat()` (consumes 1 normal cat). Counts toward the happiness denominator and the total population (`get_total_cats()` = `cats + cyborg_cats`) |
| `next_cyborg_cost` | `float` | `Config.CYBORG_COST_BASE` | Money cost of the next cyborg conversion ($250 base); multiplied by `Config.CYBORG_COST_GROWTH` (1.4×) after each conversion. The conversion button cost label reads from this var |
| `cyborg_levels` | `Array[int]` | `[]` | Per-cyborg level list; one `int` (1/2/3) per cyborg, appended in creation order by `buy_cyborg_cat()`. Invariant: `cyborg_levels.size() == cyborg_cats` (guarded by `assert` in `buy_cyborg_cat()`). Source of truth for the income sum in `get_onlypaws_cyborg_earning_units()` |
...
| `get_research_cats` | `() -> int` | Returns `get_research_cats_for(cats)` (= `floor(cats * research_cat_fraction)`); number of normal cats assigned to research. Drives research-point accumulation (cyborgs do not contribute research points) |
| `get_onlypaws_cats` | `() -> int` | Returns `cats - get_research_cats()`; number of normal cats contributing to OnlyPaws income. Kept intact for callers; income now uses the pool-aware helpers below |
| `get_research_cats_for` | `(pool: int) -> int` | Returns `floor(pool * research_cat_fraction)`; splits any population pool proportionally so normals and cyborgs share the same research fraction |
| `get_onlypaws_normal_cats` | `() -> int` | Returns `cats - get_research_cats_for(cats)`; normal cats earning OnlyPaws income |
| `get_onlypaws_cyborg_cats` | `() -> int` | Returns `cyborg_cats - get_research_cats_for(cyborg_cats)`; cyborg cats earning OnlyPaws income (used for population counts; income uses `get_onlypaws_cyborg_earning_units()`) |
| `get_onlypaws_cyborg_earning_units` | `() -> float` | Returns the research-split, multiplier-weighted cyborg income contribution. Of `cyborg_levels.size()` cyborgs, `get_research_cats_for(cyborg_levels.size())` are skipped (assigned to research — first N by insertion order); the rest contribute `sum(Config.CYBORG_MULTIPLIERS[level-1])`. Used by `update_paws_rate()` and both no-bot fallbacks |
| `count_cyborgs_eligible_to_level` | `() -> int` | Returns the count of cyborgs eligible to advance: V1s if `research_complete["cyborg_level_2"]`, V2s if `research_complete["cyborg_level_3"]` |
| `level_up_cyborg` | `() -> int` | Scans `cyborg_levels` in order for the first eligible cyborg (V1 if L2 done, V2 if L3 done); increments its level, calls `update_paws_rate()`, emits `cyborg_leveled(old, new)`. Returns the new level (2 or 3), or 0 if none eligible |
| `get_total_cats` | `() -> int` | Returns `cats + cyborg_cats`; the total population used for the "Cats: X/MAX" HUD count and the housing cap, so converting a cat keeps the total unchanged and a cyborg still occupies a slot |
| `buy_cyborg_cat` | `() -> void` | Guards `research_complete["cyborg_cats"]`, `cats >= 1`, `money >= next_cyborg_cost`; deducts money, `cats -= 1`, `cyborg_cats += 1`, `cyborg_levels.append(1)`, asserts `cyborg_levels.size() == cyborg_cats`, multiplies `next_cyborg_cost` by `Config.CYBORG_COST_GROWTH`, calls `update_paws_rate()`, emits `cyborg_cat_created` |
...
| `update_paws_rate` | `() -> void` | Computes `rate = onlypaws_income_per_cat + onlypaws_income_per_bot * manager_bots + MEGA_BOT_INCOME_PER_CAT * mega_bots`, then `paws_income_rate = rate * (get_onlypaws_normal_cats() + get_onlypaws_cyborg_earning_units())`. Worked example: 20 normal + 10 cyborg (all V2, M=4), 10 bots, 5 mega-bots, fraction 0.0 → rate=10.25, cyborg_units=10×4=40, income=10.25×(20+40)=615/sec. Must be called whenever cat/cyborg count, bot/mega-bot count, cyborg levels, or `research_cat_fraction` change. The no-bot fallback in both `_process()` calls mirrors this with only the base per-cat rate |
...
|---|---|---|---|
| `RESEARCH_ITEMS` | `Array` | 6 entries | Research item definitions. Each entry: `{id, name, subtitle, description, fund_cost: float, points_cost: float, min_cats_required: int, cat_intelligence_gain: int, min_housing_tier: int}`; some entries add optional `unlock_requires_cats: int` / `unlock_requires_research: String` keys. `min_housing_tier` is one of the gates on panel visibility in Main.gd's `_refresh_research_slots()` queued slot system (housing-tier gate + predecessor-complete gate + optional OR unlock gate + `Config.RESEARCH_MAX_VISIBLE` cap). Items in order: `cat_power_unite` ($1,000 fund, 200 pts, 10 cats, +1 cat_intelligence, min_housing_tier 0); `ai_model_upgrade` ($2,000 fund, 1,200 pts, 1 cat, min_housing_tier 1; unlocks Mega Manager-Bots); `robo_shit_sweeper` ($4,000 fund, 2,400 pts, 1 cat, min_housing_tier 0; OR-gate: cats>=25 or ai_model_upgrade done); `cyborg_cats` ($3,000 fund, 1,800 pts, 1 cat, min_housing_tier 2, unlock_requires_cats:20; completing it unlocks the Cyborg Cats conversion UI and `_level_up_cyborg_button`); `cyborg_level_2` ($6,000 fund, 3,000 pts, 1 cat, min_housing_tier 2; completing it allows any V1 cyborg to be leveled to V2); `cyborg_level_3` ($12,000 fund, 5,000 pts, 1 cat, min_housing_tier 2; completing it allows any V2 cyborg to be leveled to V3). The `cyborg_level_2`/`cyborg_level_3` items use only the index-order predecessor gate — their panel only appears after all prior items (including `cyborg_cats` / `cyborg_level_2` respectively) are complete. The `name`/`subtitle`/`description` values reference `Strings.RESEARCH_*` consts rather than inline literals. |
...
| `CYBORG_COST_BASE` | `float` | `250.0` | Money cost of the first cyborg conversion; seeds `GameState.next_cyborg_cost` |
| `CYBORG_COST_GROWTH` | `float` | `1.4` | Multiplier applied to `next_cyborg_cost` after each conversion |
| `CYBORG_MULTIPLIERS` | `Array` | `[2.0, 4.0, 8.0]` | Cyborg income multiplier per level; indexed by `level - 1` (V1→2×, V2→4×, V3→8×) |
...
| `housing_tiers` | `Array` | 5 entries | Housing upgrade chain; each entry has `id`, `label`, `cost`, `max_cats_increase`; labels reference `Strings.HOUSING_LABEL_*`; costs: 0 / 500 / 3.5k / 11.5k / 46k; tier 1 label is "Luxury Cat Trees" |
...
- **Housing tier labels** (`BTN_HOUSING_UPGRADE` = `"%s\n$%s"` template for the shop button; `HOUSING_LABEL_STUDIO_BASIC`, `HOUSING_LABEL_STUDIO_UPGRADED`, `HOUSING_LABEL_BEDROOM_1/2/3`) — referenced by `Config.housing_tiers` array and displayed in Main.gd's housing shop button via `Strings.BTN_HOUSING_UPGRADE % [label, cost]`.
...
- **Cyborg Cats** (`HUD_CYBORG_CATS` = "Cyborg Cats: %s", `BTN_MAKE_CYBORG` = "Make Cyborg Cat ($%s) [-1 cat]", `BTN_LEVEL_UP_CYBORG` = "Level Up Cyborg → V%s") — live count label, per-frame conversion button cost, and level-up shop button label.
...
| `_on_make_cyborg_button_pressed()` | Calls `GameState.buy_cyborg_cat()` |
| `_on_level_up_cyborg_button_pressed()` | Calls `GameState.level_up_cyborg()` |
| `_on_cyborg_popup_ok_pressed()` | Hides `CyborgPopup` and unpauses the tree |
| `_on_cyborg_cat_created()` | Reacts to `GameState.cyborg_cat_created`: finds the first CatContainer child not already in `_cyborg_cat_ids`, records its id → level 1, tints it `CYBORG_TINTS[0]` (sky blue), and erases it from `_cat_poop_timers` so cyborgs never poop |
| `_on_cyborg_leveled(old_level, new_level)` | Reacts to `GameState.cyborg_leveled`: finds the first `_cyborg_cat_ids` entry matching `old_level`, bumps it to `new_level`, and re-tints the corresponding CatContainer child to `CYBORG_TINTS[new_level - 1]` (V2 = violet, V3 = amber) |
...
| `_spawn_sweeper_instance()` | Builds one sweeper Node2D + Label (emoji `Strings.SWEEPER_EMOJI`, font 40, centered at -20,-20), adds it to Main, and appends a `{node, label, state: MOVING, target: {}, clean_timer: 0.0}` dict to `_sweepers`. Called by `_process()` inside a `while _sweepers.size() < GameState.robo_sweeper_count` loop so newly-purchased sweepers appear the same frame they are bought |
...
- [x] **Cyborg Cats** — gated behind the `cyborg_cats` research (min_housing_tier 2, unlocks at 20 cats, $3,000 fund / 1,800 pts, all prior research complete). Once complete, a "Resistance Is Fur-tile" achievement popup fires once and the conversion UI (CyborgCatsLabel + MakeCyborgButton) appear. `buy_cyborg_cat()` converts 1 normal cat into a V1 cyborg for an escalating money cost (`CYBORG_COST_BASE` $250 × `CYBORG_COST_GROWTH` 1.4 per conversion). Each cyborg has its own level (1/2/3) stored in `GameState.cyborg_levels`; income = `Config.CYBORG_MULTIPLIERS[level-1]` × the full per-cat rate, summed per-cyborg (`get_onlypaws_cyborg_earning_units()`). Cyborgs never poop (tinted in CatContainer — V1 sky blue, V2 violet, V3 amber), still consume cat food, and count toward happiness denominator. The research/OnlyPaws slider split applies proportionally to both pools. Cyborgs are leveled V1→V2 / V2→V3 by completing `cyborg_level_2` / `cyborg_level_3` research (no money cost) and clicking the `_level_up_cyborg_button` (one cyborg per click, by scan order)
