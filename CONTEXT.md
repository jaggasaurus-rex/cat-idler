...
| `cyborg_cats` | `int` | `0` | Cyborg cat population, **separate** from `cats`. Each earns `get_cyborg_multiplier()` × the full per-cat income rate and never poops. Created by `buy_cyborg_cat()` (consumes 1 normal cat). Counts toward the happiness denominator (`cats + cyborg_cats`) but not toward `cats`/`get_max_cats()` |
