...
│   │       ├── HappinessBarContainer (VBoxContainer) ← always visible; Cat Happiness title + bar
│   │       │   ├── HappinessTitleLabel (Label "Cat Happiness")
│   │       │   └── HappinessRow (HBoxContainer)
│   │       │       ├── HappinessMinLabel (Label "0%")
│   │       │       ├── HappinessBar (ProgressBar) ← fill colour interpolated red→green each frame
...
│           └── CenterColumn (VBoxContainer) ← research section; hidden until housing_tier_index >= 1
│               ├── ResearchTitle, ResearchActiveLabel, ResearchProgressBar
│               ├── ResearchSlider (hidden until first item funded)
│               ├── SliderLabels (OnlyPawsHint | ResearchHint)
│               ├── ResearchCatsLabel
│               └── ResearchItemList (VBoxContainer) ← dynamically populated in _ready()
...
| `_on_cat_purchased()` | Instantiates `CatCharacter` at scale 0.4, adds to `CatContainer`, calls `cat.set_bounds(center_panel.get_global_rect())` to inject the play-area rect, calls `_place_cat(cat)`, and registers the cat in `_cat_bubble_timers` and `_cat_poop_timers` with initial random cooldowns |
...
| `_place_cat(cat: Node2D)` | Places a newly added cat at a random position inside `center_panel.get_global_rect()`: 40 px inset from all CenterPanel edges, top 10% excluded so bubbles don't clip above the panel. Up to 30 attempts to avoid existing cats (64 px radius). Falls back to an unconstrained position within the same bounds. UI-rect avoidance no longer needed — all UI is outside CenterPanel. |
...
- [x] **Cat spawning** — each purchase instances `CatCharacter` at scale 0.4 into `CatContainer` (child of CenterPanel); `set_bounds(center_panel.get_global_rect())` is injected at spawn; placed at a random position within CenterPanel bounds (40 px inset, top 10% excluded) avoiding other cats (64 px radius)
