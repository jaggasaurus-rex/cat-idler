...
| `_viral_delay_timer` | `float` | `0.0` | Private accumulator; counts up in `_process()` once `manager_bots >= 1` toward the 20s viral-bubble unlock delay |
| `viral_bubbles_unlocked` | `bool` | `false` | One-way latch; set to `true` in `_process()` when `manager_bots >= 1` and `_viral_delay_timer >= 20.0`; gates all bubble spawning in Main.gd `_try_spawn_bubble_for_cat()` |
| `viral_popup_shown` | `bool` | `false` | One-way latch; set to `true` in Main.gd `_spawn_bubble()` the first time a viral bubble would spawn; that first event fires the whale popup instead of a bubble |
...
Drives the root scene. Reads from and delegates to `GameState`; the only local mutable state is UI-latch flags plus the bubble mechanic's `_cat_bubble_timers` (Dictionary: cat `instance_id` → seconds remaining) and `_active_bubbles` (Array of `{node, timer, type, research_id}` dicts).
...
| `_on_cat_purchased()` | Instantiates `CatCharacter` at scale 0.4, adds to `CatContainer`, calls `_place_cat(cat)`, and registers the cat in `_cat_bubble_timers` with an initial `randf_range(BUBBLE_SPAWN_MIN, BUBBLE_SPAWN_MAX)` cooldown |
| `_on_cat_lost()` | Removes the last `CatContainer` child: erases its `instance_id` from `_cat_bubble_timers`, then `queue_free()`s the node |
...
| `_spawn_bubble(cat_node)` | Picks bubble type: `"viral"` when no research is active (`GameState.get_active_research_id() == ""`); otherwise `"inspiration"` with probability `research_cat_fraction`, else `"viral"`. If the type is `"viral"` and `GameState.viral_popup_shown` is false, sets it true, calls `_show_viral_popup()`, and returns without spawning (the popup is the first viral event). Otherwise creates a clickable Button over `cat_node` (text `"💰"` viral / `"💡"` inspiration; `font_size` override 48, `custom_minimum_size` 80×80), positioned at `cat_node.global_position + Vector2(randf_range(-30,30), randf_range(-50,-20))`, `z_index = 100` (renders in front of all UI panels), child of Main; stores a `{node, timer, type, research_id}` dict in `_active_bubbles` and binds `gui_input` → `_on_bubble_gui_input(event, bubble)`. `research_id` is captured at spawn so collection still works if research changes mid-flight |
| `_show_viral_popup()` | Builds the one-time "Whale Hunting Baby!" achievement popup entirely in code (full-screen `ColorRect` overlay with `process_mode = WHEN_PAUSED` and `z_index = 20` → `CenterContainer` → `PanelContainer` → `VBoxContainer` with an autowrapped `Label` and OK `Button`); pauses the tree on show; OK button frees the overlay and unpauses |
...
- [x] **Bubble mechanic** — clickable floating Button bubbles spawn over individual cats. Each cat runs its own randomized cooldown (`_cat_bubble_timers`, keyed by `instance_id`) of `randf_range(Config.BUBBLE_SPAWN_MIN, BUBBLE_SPAWN_MAX)` = 5–15s, registered on purchase and erased on loss; when a cat's timer expires it attempts a spawn anchored to that cat. Capped at `BUBBLE_MAX_ON_SCREEN` (4) total. Gated behind `viral_bubbles_unlocked` (set 20s after the first Manager-Bot is owned) and `only_paws_active`. Two types: Viral (💰, reward = `paws_income_rate × 4`, min $1) and Inspiration (💡, reward = `get_research_cats() × 3` research points, min 1, clamped to `points_cost`). Viral always spawns when no research is active; with active research, Inspiration spawns with probability `research_cat_fraction` else Viral. The first viral spawn fires the one-time "Whale Hunting Baby!" achievement popup (built in code, pauses tree) instead of a bubble; bubbles flow normally after dismissal. Bubbles fade out over `BUBBLE_LIFETIME` (4.5s) and disappear if not clicked. Bubble Buttons are large (font 48, 80×80 min size) and use `z_index = 100` to render in front of all UI; a left-click collects the clicked bubble plus any other bubbles stacked under the cursor in one click (via `gui_input` + `get_global_rect().has_point`). Created in code (no scene file), children of Main
