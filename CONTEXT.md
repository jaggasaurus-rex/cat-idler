...
Drives the root scene. Reads from and delegates to `GameState`; the only local mutable state is UI-latch flags plus the bubble mechanic's `_cat_bubble_timers` (Dictionary: cat `instance_id` → seconds remaining), `_active_bubbles` (Array of `{node, timer, type, research_id}` dicts), and the global burst-window state (`_burst_window_active`, `_burst_window_timer`, `_global_cd_timer`).
