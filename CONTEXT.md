...
| `cat_cost_growth_rate` | `float` | `Config.cat_cost_growth_rate` | Multiplier applied to `next_cat_cost` each purchase; capped at `Config.breeder_contract_growth_rate` (1.25) by the shop Breeder Contract (no-ops when research reductions have already brought it below 1.25); multiplied by 0.9 / 0.8 on `cat_breeder_contract` / `cat_breeders_contract` research completion; may fall below 1.0 when both compound |
...
| `multiply_cat_cost_growth` | `(factor: float) -> void` | Multiplies `cat_cost_growth_rate` by `factor`, then retroactively recalculates `next_cat_cost = max(1.0, Config.cat_cost_base * pow(cat_cost_growth_rate, float(cats)))`; called with 0.9 on `cat_breeder_contract` completion and 0.8 on `cat_breeders_contract` completion; stacks correctly with `buy_breeder_contract()` in any order (shop uses `min()` so it never raises a rate the research already lowered); rate may fall below 1.0 when both reductions compound (1.3 × 0.9 × 0.8 ≈ 0.94), making each successive cat cheaper than the last; `next_cat_cost` floored at $1.00 to prevent retroactive recalculation collapsing to near-zero with high cat counts; `buy_cat()` applies the same floor after each purchase |
