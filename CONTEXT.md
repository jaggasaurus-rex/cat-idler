...
| `get_happiness` | `() -> float` | **Decay stripped (mechanic being redesigned): now always returns `100.0`.** The over-max two-segment quadratic decay was removed; with happiness pinned at 100 the over-max happiness mechanic is neutralised and a new **hard cat cap** in `buy_cat()` (`cats >= get_max_cats()`) prevents the player from exceeding max_cats at all. The happiness bar, all happiness popups, and all happiness state variables remain in place but are now inert. The income happiness multiplier evaluates to 1.0 automatically. |
