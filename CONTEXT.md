...
| `CAT_WANDER_MIN` | `float` | `6.0` | Min seconds between a cat's movement decisions |
| `CAT_WANDER_MAX` | `float` | `18.0` | Max seconds between a cat's movement decisions |
...
- [x] **Cat wandering** — each `CatCharacter` autonomously wanders: after a `randf_range(CAT_WANDER_MIN, CAT_WANDER_MAX)` = 6–18s idle, it picks a random destination inside the safe zone (40px inset, top 10% excluded) and walks there at `CAT_MOVE_SPEED` = 40 px/s, flipping its sprite toward the target and playing `"walk"`/`"idle"` animations by name (no-ops until those frames are authored in the editor; only `"default"` exists today). Movement and the wander timer freeze while a viral money bubble is active above the cat
