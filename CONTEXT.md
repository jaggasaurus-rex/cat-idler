...
| `robo_sweeper_purchased` | `bool` | `false` | One-way latch; set by `buy_robo_sweeper()` once `research_complete["robo_shit_sweeper"]` is true and the player can afford `Config.ROBO_SWEEPER_PURCHASE_COST`. Will wire into the poop/happiness system in a future task; currently only gates its shop button visibility in Main.gd |
